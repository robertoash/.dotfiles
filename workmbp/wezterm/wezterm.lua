...
-- Detect OS and set appropriate fish path
local fish_path = "fish" -- fallback
local nix_paths = ""

if wezterm.target_triple:find("darwin") then
	-- macOS (nix-darwin)
	fish_path = "/etc/profiles/per-user/rash/bin/fish"
	nix_paths = "/etc/profiles/per-user/rash/bin:/run/current-system/sw/bin:/nix/var/nix/profiles/default/bin"
else
	-- Linux/NixOS
	fish_path = "/run/current-system/sw/bin/fish"
	nix_paths = "/run/current-system/sw/bin:/home/rash/.nix-profile/bin:/nix/var/nix/profiles/default/bin"
end

-- Always use fish shell as default
config.default_prog = { fish_path }

-- Set environment variables to include Nix paths
config.set_environment_variables = {
	PATH = nix_paths .. ":/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin",
}
...
config.keys = {
	...
	-- Cmd+Left/Right → Beginning/End of line (catches Karabiner's Home/End → Cmd+Left/Right mapping)
	{
		key = "LeftArrow",
		mods = "CMD",
		action = wezterm.action.SendString("\x1bOH"), -- Same as Home key
	},
	{
		key = "RightArrow",
		mods = "CMD",
		action = wezterm.action.SendString("\x1bOF"), -- Same as End key
	},

	-- Home and End keys to move to beginning/end of line (fallback if Karabiner isn't running)
	{
		key = "Home",
		mods = "NONE",
