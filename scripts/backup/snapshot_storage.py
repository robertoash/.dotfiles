...
def create_snapshot(snapshot_name):
    ...
    subprocess.run(["sudo", "btrfs", "subvolume", "snapshot", "-r", "/", f"{local_snapshot_dir}/{snapshot_name}"], check=True)
...
def send_snapshot(snapshot_name):
    ...
    if not os.path.isdir(f"{local_snapshot_dir}/{snapshot_name}"):
        print(f"Snapshot directory {local_snapshot_dir}/{snapshot_name} does not exist.")
        return
...
def receive_snapshot(snapshot_name):
    ...
    with subprocess.Popen(["sudo", "btrfs", "send", os.path.join(external_drive_dir, snapshot_name)], stdout=subprocess.PIPE) as send_proc:
        subprocess.run(["sudo", "btrfs", "receive", local_snapshot_dir], stdin=send_proc.stdout, check=True)
    print(f"Snapshot {snapshot_name} received from external drive successfully.")

def main():
    parser = argparse.ArgumentParser(description="Manage Btrfs snapshots between local system and external drive.")
    parser.add_argument("-s", "--save", action="store_true", help="Save a snapshot of the root filesystem to an external drive.")
    parser.add_argument("-r", "--recall", action="store_true", help="Recall a snapshot from an external drive to the local system.")
